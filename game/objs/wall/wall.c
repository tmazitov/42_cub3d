...
/*   Updated: 2024/08/24 17:26:21 by tmazitov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_wall	*make_wall(int x, int y, t_wall_type type, t_direction direction)
{
	t_wall	*wall;
...
{
	return (make_line(wall->start->x, wall->start->y, wall->end->x,
			wall->end->y));
}
...
t_line	*wall_to_direction_line(t_wall *wall, t_direction direction)
{
	if (wall->direction == WEST && direction == NORTH)
		return (make_line(wall->start->x - 64, wall->start->y, wall->start->x,
				wall->start->y));
	if (wall->direction == WEST && direction == SOUTH)
		return (make_line(wall->end->x, wall->end->y, wall->end->x - 64,
				wall->end->y));
	...
	if (wall->direction == NORTH && direction == WEST)
		return (make_line(wall->start->x, wall->start->y, wall->start->x,
				wall->start->y - 64));
	if (wall->direction == NORTH && direction == EAST)
		return (make_line(wall->end->x, wall->end->y, wall->end->x, wall->end->y
				- 64));
