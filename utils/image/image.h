...
/*   Updated: 2024/08/25 23:10:31 by tmazitov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct s_image_put_step
{
	int						dest_x;
	int						dest_y;
	int						src_x;
...
void						img_put_img(t_image *dest, t_image *src,
								t_point pos, double angle);
void						img_put_pixel(t_image *img, int color, int x,
								int y);
void						img_put_rectangle(t_image *img, t_rectangle rect,
								int color);

void						img_paint_border(t_image *image,
								t_image_border border);

/* ANIMATION PART */
/* Provide you to create your own animation.
-----------------------------------------------------------
Using function `make_anime` you can create 
(with memory allocation) animation instance. 

*  To add an animation frame you can use `anime_add_frame` 
with animation instance,image and future frame duration. 

* To free allocated memory of an animation instance you can use `free_anime`. 
...
int							anime_add_frame(t_anime *anime, t_image *image,
								int duration);
